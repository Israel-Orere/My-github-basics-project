...
async function main(){
 ...
 const r=await fetch(url);if(!r.ok)throw new Error(`candle endpoint ${symbol} returned ${r.status}: ${await r.text()}`);const j:any=await r.json();const rows:any[]=j.candles||j.data||j;
