...
function fallback(prompt:string,error?:unknown):StrategySpec{
 const s=compileStrategy(prompt),lower=prompt.toLowerCase(),advanced=/\brsi\b|\bema\b|\bsma\b|moving average|\bmacd\b|\batr\b|stoch|bollinger|\bvwap\b|\bobv\b|\broc\b|\bcross(?:es|ing)?\b|previous (high|low|close)|\bvolume\b/.test(lower),category=agentFailureCategory(error);
