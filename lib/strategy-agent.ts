...
const ValueExprSchema=z.object({
 kind:z.enum(['CONSTANT','PRICE','OPEN','HIGH','LOW','VOLUME','RSI','SMA','EMA','MACD','MACD_SIGNAL','ATR','ROC','STOCH_K','STOCH_D','BB_UPPER','BB_MIDDLE','BB_LOWER','VWAP','OBV','HIGHEST_HIGH','LOWEST_LOW','RANGE_WIDTH','RANGE_POSITION']),
...
- HIGHEST_HIGH(period): highest candle high in the rolling window
- LOWEST_LOW(period): lowest candle low in the rolling window
- RANGE_WIDTH(period): HIGHEST_HIGH(period) minus LOWEST_LOW(period)
- RANGE_POSITION(period): current PRICE expressed as 0-100 within a rolling high-low range. For this primitive only, offsetBars shifts the range window while keeping the current price as the numerator. RANGE_POSITION(period=20,offsetBars=1) therefore means current price position inside the previous 20 completed bars' high-low range. Values can be below 0 or above 100 if price is outside that prior range.
- CONSTANT
Every non-constant expression may specify timeframe, offsetBars, multiplier and addend. For ordinary expressions, offsetBars=0 means the current completed bar and offsetBars=1 means the previous completed bar. For RANGE_POSITION, offsetBars shifts only the reference range as described above. Use multiplier/addend to express rules such as price > EMA*1.02 or fastEMA > slowEMA+5.
...
- “inside the previous N-bar high-low range” means current PRICE <= HIGHEST_HIGH(N, offsetBars=1) AND current PRICE >= LOWEST_LOW(N, offsetBars=1).
- “inside the lower third of the previous N-bar range” means RANGE_POSITION(N, offsetBars=1) >= 0 AND RANGE_POSITION(N, offsetBars=1) <= 33.333.
- “inside the upper 25% of the previous N-bar range” means RANGE_POSITION(N, offsetBars=1) >= 75 AND <= 100.
- “break above the previous N-bar high” means PRICE crosses ABOVE HIGHEST_HIGH(N, offsetBars=1).
- If the user says “previous highs and lows” but does not specify how many bars and the count materially changes the rule, ask for the lookback instead of inventing one.
...
function expressionProblems(e:ValueExpr,path:string){const out:string[]=[];if(e.kind==='CONSTANT'&&!Number.isFinite(e.value))out.push(`${path}: CONSTANT needs a numeric value`);if(['SMA','EMA','ATR','ROC','STOCH_K','STOCH_D','BB_UPPER','BB_MIDDLE','BB_LOWER','VWAP','HIGHEST_HIGH','LOWEST_LOW','RANGE_WIDTH','RANGE_POSITION'].includes(e.kind)&&!(e.period&&e.period>0))out.push(`${path}: ${e.kind} needs period`);if(e.kind==='RSI'&&!(e.period&&e.period>0))out.push(`${path}: RSI needs period`);if((e.kind==='SMA'||e.kind==='EMA')&&e.source==='RSI'&&!(e.sourcePeriod&&e.sourcePeriod>0))out.push(`${path}: ${e.kind} of RSI needs sourcePeriod`);if((e.kind==='MACD'||e.kind==='MACD_SIGNAL')&&(!(e.fastPeriod&&e.slowPeriod)||Number(e.fastPeriod)>=Number(e.slowPeriod)))out.push(`${path}: MACD needs fastPeriod < slowPeriod`);if(e.kind==='MACD_SIGNAL'&&!(e.signalPeriod&&e.signalPeriod>0))out.push(`${path}: MACD_SIGNAL needs signalPeriod`);return out}
...
function agentFailureCategory(error:unknown){const raw=error instanceof Error?`${error.name}: ${error.message}`:String(error||'unknown');const text=raw.toLowerCase();if(/401|403|unauthor|forbidden|auth|api.?key|oidc/.test(text))return'gateway-auth';if(/402|429|quota|credit|billing|rate.?limit/.test(text))return'gateway-quota';if(/404|model.*not|unknown model|model id|routing/.test(text))return'model-routing';if(/schema|structured|json|object|output|validation|parse/.test(text))return'structured-output';if(/timeout|network|fetch|connection|socket|dns/.test(text))return'gateway-network';return'agent-request'}
function fallback(prompt:string,error?:unknown):StrategySpec{
 const s=compileStrategy(prompt),lower=prompt.toLowerCase(),advanced=/\brsi\b|\bema\b|\bsma\b|moving average|\bmacd\b|\batr\b|stoch|bollinger|\bvwap\b|\bobv\b|\broc\b|\bcross(?:es|ing)?\b|previous (high|low|close)|highest high|lowest low|high[- ]low range|range position|\bvolume\b/.test(lower),category=agentFailureCategory(error);
 return {...s,conditionGroups:[{logic:'ALL',conditions:s.trigger.streakLength?[{type:'SETTLEMENT_STREAK',side:s.trigger.streakSide||s.side,length:s.trigger.streakLength,label:`Previous ${s.trigger.streakLength} contracts settled ${s.trigger.streakSide||s.side}`}]:[]}],compiler:'deterministic-fallback',interpretation:{summary:advanced?'This strategy needs the agent compiler before DreamForge can backtest it safely.':`${s.asset} ${s.window} ${s.side} event-contract strategy.`,entry:s.trigger.streakLength?[`Wait for ${s.trigger.streakLength} consecutive ${s.trigger.streakSide} settlements.`]:[],execution:[`Buy ${s.side} only at ${Math.round((s.trigger.maxEntryPrice||.99)*100)}¢ or less.`],sizing:[`Start at $${s.sizing.baseUsd}; after win $${s.sizing.afterWinUsd}; after loss $${s.sizing.afterLossUsd}.`],risk:[`Stop at -$${s.risk.maxLossUsd} or ${s.risk.maxTrades} trades within ${s.risk.durationHours}h.`],assumptions:[],questions:advanced?[`The AI strategy compiler is temporarily unavailable (${category}). DreamForge will not backtest or activate this complex strategy until the agent compiler succeeds.`]:[],confidence:advanced?0:.7,needsClarification:advanced}};
}
...
export async function compileStrategyWithAgent(prompt:string):Promise<StrategySpec>{
 ...
 }catch(e){console.error('strategy-agent fallback',e);return fallback(prompt,e)}
}
