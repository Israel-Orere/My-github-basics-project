...
function ema(src:number[],period:number){const out=Array(src.length).fill(NaN);if(!src.length)return out;const k=2/(period+1);let start=src.findIndex(Number.isFinite);if(start<0)return out;let seed:number[]=[];for(let i=start;i<src.length;i++){if(Number.isFinite(src[i]))seed.push(src[i]);if(seed.length===period){let prev=seed.reduce((a,b)=>a+b,0)/period;out[i]=prev;for(let j=i+1;j<src.length;j++){if(!Number.isFinite(src[j]))continue;prev=src[j]*k+prev*(1-k);out[j]=prev}break}}return out}
...
export function createStrategyEvaluator(strategy:StrategySpec,barsByTf:BarsByTimeframe){
 ...
 function series(e:ValueExpr):number[]{
  ...
  else if(e.kind==='SMA'||e.kind==='EMA'){
   const source=e.source==='RSI'?rsi(closes,e.sourcePeriod||14):closes;out=e.kind==='SMA'?sma(source,e.period||20):ema(source,e.period||20);
  }else{
   const fast=ema(closes,e.fastPeriod||12),slow=ema(closes,e.slowPeriod||26),macd=closes.map((_,i)=>finite(fast[i])&&finite(slow[i])?fast[i]-slow[i]:NaN);out=e.kind==='MACD'?macd:ema(macd,e.signalPeriod||9);
  }
  seriesCache.set(k,out);return out;
 }
 function indexBefore(e:ValueExpr,time:number){const bars=barsFor(e);let lo=0,hi=bars.length-1,ans=-1;while(lo<=hi){const mid=(lo+hi)>>1;if(bars[mid].time<time){ans=mid;lo=mid+1}else hi=mid-1}return ans}
