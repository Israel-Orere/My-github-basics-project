...
function ChatBubble({message}:{message:ChatMessage}){
  ...
  return <div className={`${styles.bubble} ${styles.agentBubble}`}><small>DREAMFORGE</small>{r?<>
    ...
    {r.understood?.length>0&&<div className={styles.chatRules}>{r.understood.slice(0,12).map((x,i)=><div key={i}><span>✓</span><p>{x}</p></div>)}</div>}
    ...
    <div className={styles.confidenceRow}><span>{Math.round((r.confidence||0)*100)}% interpretation confidence</span><span>{compilerLabel(r.compiler)}</span></div>
...
function ChartView({data}:{data:ChartData}){
  ...
  return <div className={styles.chartWrap}>
    ...
    <div className={styles.legend}>{data.bands?.length>0&&<span><i className={styles.rangeSwatch}/>Rolling range</span>}{(data.overlays||[]).slice(0,5).map((o,i)=><span key={o.id}><i className={`${styles.lineSwatch} ${styles[`legend${i+1}` as keyof typeof styles]||''}`}/>{o.label}</span>)}{data.markers?.length>0&&<span><i className={styles.markerSwatch}/>Technical setup match</span>}</div>
  </div>;
}

function latestValue(values:(number|null)[]){for(let i=values.length-1;i>=0;i--)if(typeof values[i]==='number'&&Number.isFinite(values[i]))return values[i] as number;return null}
function indicatorNumber(v:number){const a=Math.abs(v);if(a>=1000000)return`${(v/1000000).toFixed(2)}m`;if(a>=1000)return v.toLocaleString(undefined,{maximumFractionDigits:1});if(a>=100)return v.toFixed(1);if(a>=10)return v.toFixed(2);return v.toFixed(3)}
function IndicatorPane({panel,count}:{panel:ChartPanel;count:number}){
  const model=useMemo(()=>{const vals:number[]=[];for(const s of panel.series)for(const v of s.values)if(typeof v==='number'&&Number.isFinite(v))vals.push(v);for(const r of panel.references||[])if(Number.isFinite(r.value))vals.push(r.value);if(!vals.length)return null;let min=Math.min(...vals),max=Math.max(...vals);if(['RSI','STOCH','RANGE_POSITION'].includes(panel.key)){min=0;max=100}else{const span=Math.max(Math.abs(max-min),Math.abs(max)*.03,1e-6),pad=span*.1;min-=pad;max+=pad;if(min===max){min-=1;max+=1}}const w=900,h=142,left=18,right=72,top=12,bottom=18,plotW=w-left-right,plotH=h-top-bottom,x=(i:number)=>left+(count<=1?plotW/2:(i/(count-1))*plotW),y=(v:number)=>top+((max-v)/(max-min))*plotH;return{min,max,w,h,left,right,top,bottom,plotW,plotH,x,y}},[panel,count]);
  if(!model)return null;const{min,max,w,h,left,top,plotW,plotH,x,y}=model;
  return <div className={styles.indicatorPane}><div className={styles.indicatorHead}><b>{panel.label}</b><div>{panel.series.map((s,i)=>{const v=latestValue(s.values);return <span key={s.id}><i className={`${styles.lineSwatch} ${styles[`legend${(i%5)+1}` as keyof typeof styles]||''}`}/>{s.label}{v!==null?` · ${indicatorNumber(v)}`:''}</span>})}</div></div><svg className={styles.indicatorChart} viewBox={`0 0 ${w} ${h}`} role="img" aria-label={`${panel.label} indicator pane`}><rect x={left} y={top} width={plotW} height={plotH} className={styles.plotBg}/>{[0,.5,1].map((p,i)=><line key={i} x1={left} x2={left+plotW} y1={top+p*plotH} y2={top+p*plotH} className={styles.gridLine}/>)}{min<0&&max>0&&<line x1={left} x2={left+plotW} y1={y(0)} y2={y(0)} className={styles.zeroLine}/>} {(panel.references||[]).map((r,i)=><g key={`${r.value}-${i}`}><line x1={left} x2={left+plotW} y1={y(r.value)} y2={y(r.value)} className={styles.referenceLine}/><text x={left+plotW+7} y={y(r.value)+3} className={styles.referenceText}>{indicatorNumber(r.value)}</text></g>)}{panel.series.map((s,i)=>{const pts=s.values.map((v,j)=>typeof v==='number'?`${x(j)},${y(v)}`:null).filter(Boolean).join(' ');return <polyline key={s.id} points={pts} className={`${styles.overlay} ${styles[`overlay${(i%5)+1}` as keyof typeof styles]||''}`}><title>{s.label}</title></polyline>})}<text x={left+plotW+7} y={top+9} className={styles.axisText}>{indicatorNumber(max)}</text><text x={left+plotW+7} y={top+plotH} className={styles.axisText}>{indicatorNumber(min)}</text></svg></div>
}
