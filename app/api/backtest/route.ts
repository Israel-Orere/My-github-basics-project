...
function firstTouch(rows:CandleRow[],start:number,expiry:number,decimals:number,side:Side,cap:number){for(let i=lowerBound(rows,start);i<rows.length;i++){const c=rows[i],t=Number(c.bucketStart);if(t>=expiry)break;const lo=probability(c.low,decimals),hi=probability(c.high,decimals);if(side==='UP'?lo<=cap:(1-hi)<=cap)return t}return null}
...
export async function POST(req:Request){
 ...
 try{
  ...
  if(!markets.length){const payload={source:'dreamdex-agent-replay',from,to,markets:0,candidateMarkets:0,trades:0,wins:0,losses:0,winRate:0,pnl:0,returnPct:0,maxDrawdown:0,endingCapital:startingCapital,equity:[startingCapital],tradeLog:[],warnings:['No finalized DreamDEX markets were found for this asset, contract window and date range.']};cache.set(cacheKey,{expires:Date.now()+CACHE_TTL_MS,payload});return NextResponse.json({...payload,cached:false,durationMs:Date.now()-started})}

  const settlementById=new Map<string,Side|null>(),missing=markets.filter((m:any)=>m.winningOutcome!==0&&m.winningOutcome!==1);for(const m of markets)settlementById.set(m.marketId,m.winningOutcome===0?'UP':m.winningOutcome===1?'DOWN':null);
  ...
  const evaluator=createStrategyEvaluator(s,barsByTf),step=evaluationStepSeconds(s),prior:Side[]=[],candidates:Candidate[]=[];
  for(const m of markets){const settlement=settlementById.get(m.marketId)??null,expiry=Number(m.expiry),start=Number(m.tradingStart||expiry-intervalSec);if(!settlement)continue;let signalTime:number|null=null;for(let t=start;t<expiry;t+=step){if(evaluator.evaluate(t,prior).passed){signalTime=t;break}}if(signalTime!==null)candidates.push({market:m,signalTime});prior.push(settlement)}

  // Binary probability may meet the user's cap before the technical signal. That must not count.
  // Fetch only the post-signal window for signal-qualified contracts.
  const windowsByPool=new Map<string,{start:number;end:number}[]>();for(const c of candidates){const m=c.market,expiry=Number(m.expiry),pool=String(m.poolAddress).toLowerCase(),arr=windowsByPool.get(pool)||[];arr.push({start:c.signalTime,end:expiry});windowsByPool.set(pool,arr)}
  ...
  const trades=tradeLog.length,warnings:string[]=[];if(trades<20)warnings.push('Small sample: fewer than 20 historical trades matched this strategy.');if(historyErrors)warnings.push(`${historyErrors} historical binary-price batch${historyErrors===1?'':'es'} could not be loaded; affected candidate markets were skipped.`);if(marketsWithoutPrice)warnings.push(`${marketsWithoutPrice} signal-qualified markets had no recorded binary-contract fills after the signal, so no entry was assumed.`);if(truncated)warnings.push('History exceeded the current 20,000-market scan cap; results cover the newest available portion of the selected range.');warnings.push(`Complex signals were evaluated through each contract at ${Math.round(step/60)}-minute completed-data boundaries. Binary entry is considered only after the historical signal time; entries are conservatively charged at your maximum accepted probability. Queue position, gas and transaction latency are not modeled.`);
  const payload={source:'dreamdex-agent-replay',from:historyFrom,to,markets:markets.length,candidateMarkets:candidates.length,historyBatches:jobs.length,evaluationStepSec:step,trades,wins,losses,winRate:trades?n2(wins/trades*100):0,pnl:n2(totalPnl),returnPct:n2(totalPnl/startingCapital*100),maxDrawdown:n2(maxDD),endingCapital:n2(cash),equity,tradeLog,warnings};cache.set(cacheKey,{expires:Date.now()+CACHE_TTL_MS,payload});return NextResponse.json({...payload,cached:false,durationMs:Date.now()-started});
