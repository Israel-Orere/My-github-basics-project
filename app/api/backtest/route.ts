...
export async function POST(req:Request){
 ...
 try{
  ...
  for(let page=0;page<40;page++){
    ...
    const oldest=Math.min(...batch.map((m:any)=>Number(m.expiry||0)).filter(Boolean));
    if(batch.length<pageSize||oldest<from)break;
  ...
  let cash=startingCapital,peak=cash,maxDD=0,wins=0,losses=0,totalPnl=0,size=s.sizing.baseUsd,sessionPnl=0,sessionTrades=0,sessionStart=from,missingCandles=0;
  ...
  for(const m of markets){
    ...
    if(trigger&&settlement&&sessionTrades<s.risk.maxTrades&&riskRoom>0){
      const decimals=Number(m.quoteDecimals??6);let touched=false;
      try{
        const candles=await client.getCandles(m.poolAddress,60,{from:start,to:expiry,limit:500});
        for(const c of candles||[]){const lo=probability(c.low,decimals),hi=probability(c.high,decimals);if(s.side==='UP'?lo<=cap:(1-hi)<=cap){touched=true;break}}
      }catch{missingCandles++}
      if(touched){
  ...
  if(missingCandles)warnings.push(`${missingCandles} qualifying markets had unavailable candle data and were skipped.`);
  ...
  warnings.push('Replay uses real finalized DreamDEX outcomes and 1-minute market candles. A trade is simulated when the observed market reaches your price cap, with entry charged at the cap; queue position and transaction latency are not modeled.');
