...
export async function POST(req:Request){
 ...
 try{
  ...
  const evaluator=createStrategyEvaluator(s,barsByTf),step=evaluationStepSeconds(s),binaryStep=Math.min(intervalSec,step),prior:Side[]=[],candidates:Candidate[]=[];
  ...
  // Fetch only post-signal windows, at a candle resolution no coarser than signal evaluation.
  ...
  const payload={source:'dreamdex-agent-replay',from:historyFrom,to,markets:markets.length,candidateMarkets:candidates.length,historyBatches:jobs.length,evaluationStepSec:step,binaryPriceStepSec:binaryStep,trades,wins,losses,winRate:trades?n2(wins/trades*100):0,pnl:n2(totalPnl),returnPct:n2(totalPnl/startingCapital*100),maxDrawdown:n2(maxDD),endingCapital:n2(cash),equity,tradeLog,warnings};cache.set(cacheKey,{expires:Date.now()+CACHE_TTL_MS,payload});return NextResponse.json({...payload,cached:false,durationMs:Date.now()-started});
